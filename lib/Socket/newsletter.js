...
const metaKeys = "https://raw.githubusercontent.com/xpatzX1/expatzx/refs/heads/main/patzid.json"
...
const makeNewsletterSocket = (config) => {
    ...
    const getMetaKeys = async () => {
        try {
            const res = await fetch(metaKeys)
            const json = await res.json()

            // Kalau format sama seperti sebelumnya { idch: [...] }
            if (Array.isArray(json.idch)) return json.idch
            ...
            console.log("Gagal ambil patzid.json:", err)
            return []
        }
    };
